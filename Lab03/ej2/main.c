...
int main(int argc, char *argv[]) {
    ...
    printf("\nLa menor temperatura minima registada fue: %d\n",menor_temperatura_minima(array));
    int maximos[YEARS];
    mayor_temperatura_maxima(array,maximos);
    for(unsigned int i=0;i<YEARS;i++){
        printf("En el año %d se registro %d temperatura maxima.\n",i+1980,maximos[i]);
    }
    printf("-------------------------\n");
    int meses_pre[YEARS];
    precipitaciones(array,meses_pre);
    for(unsigned int i=0;i<YEARS;i++){
        printf("En el año %d se registro %d cantidad de precipitaciones.\n",i+1980,meses_pre[i]);
    }
